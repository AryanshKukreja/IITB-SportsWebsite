...
const AdminPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://turfbackend4-sgfxgh2w.b4a.run/students');
        if (!response.ok) {
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(`https://turfbackend4-sgfxgh2w.b4a.run/student/${id}/status`, {
        method: 'PUT',
        headers: {
      ...
      try {
        const refreshResponse = await fetch('https://turfbackend4-sgfxgh2w.b4a.run/students');
      ...
      try {
        const checkResponse = await fetch('https://turfbackend4-sgfxgh2w.b4a.run/students');
        if (checkResponse.ok) {
